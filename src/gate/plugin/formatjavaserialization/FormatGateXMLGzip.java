...
@CreoleResource(
        name = "Gzipped GATE XML format",
        isPrivate = true,
        autoinstances = {
          @AutoInstance(hidden = true)},
        comment = "Support for reading GATE documents in gzipped XML format",
        helpURL = "")
public class FormatGateXMLGzip extends XmlDocumentFormat {

  //private static final long serialVersionUID = -2394353168913311584L;
  /**
   * Default construction
   */
  public FormatGateXMLGzip() {
    super();
  }
  ...
  public Resource init() throws ResourceInstantiationException {
    MimeType mime = new MimeType("text", "gatexml+gzip");
    mimeString2ClassHandlerMap.put(mime.getType() + "/" + mime.getSubtype(), this);
    mimeString2mimeTypeMap.put(mime.getType() + "/" + mime.getSubtype(), mime);
    mimeString2mimeTypeMap.put("application/xml+gate+gzip", mime);
    suffixes2mimeTypeMap.put("gatexml.gz", mime);
    setMimeType(mime);
    return this;
  }
  ...
  protected void unpackMarkupHelper(Document dcmnt)
    ...
    try (
            ...
            GZIPInputStream gzipin = new GZIPInputStream(urlStream);
